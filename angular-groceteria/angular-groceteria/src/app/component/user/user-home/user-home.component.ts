...
export class UserHomeComponent implements OnInit{
  ...
  ngOnInit(): void{
    ...
    this.userInputUpdate.pipe(debounceTime(400), distinctUntilChanged()).subscribe(value => {
      if(value.length > 0){
        this.gservice.searchItemByName(this.searchKeyword, this.offset -1 < 0 ? 0 : this.offset - 1, this.pageSize).pipe((take(1))).subscribe((res: any) => {
  ...
  if(qty ===""){
    ...
    this.gservice.addToCart(body, item?.itemId, this.user?.userId).pipe(take(1)).subscribe(
      (res: any) => {
        console.log(res);
        if (!!res && res?.cartId) {
